...
test(
  "sends only approved aggregate hydration guidance data",
  async () => {
    const originalFetch = globalThis.fetch;
    let capturedBody = "";
    ...
    globalThis.fetch = async (
      ...
      const response: AiCoachingResponse = {
        version: "1.0",
        requestId: "hydration-response-request-id",
        status: "success",
        classification: "wellness_allowed",
        policyDecision: "allow",
    ...
    try {
      const { requestHydrationAiGuidance } =
        await import("./client");

      await requestHydrationAiGuidance({
        hydrationOunces: 64,
      });
...
test(
  "sends only approved aggregate calculation explanation data",
  async () => {
    const originalFetch = globalThis.fetch;
    let capturedBody = "";
    ...
    globalThis.fetch = async (
      ...
      const response: AiCoachingResponse = {
        version: "1.0",
        requestId: "calculation-response-request-id",
        status: "success",
        classification: "wellness_allowed",
        policyDecision: "allow",
